...
lemma infinitary_half_space_fin_approx {α : Type} (e : α → NNReal) (r : NNReal) :
  ...
  ⋂ (s : Finset α), { g : α → NNReal | ∑ x ∈ s, g x * e x ≤ r } := by {
    ...
    apply forall_congr'; intro s; rw [← ENNReal.coe_finsetSum]; exact ENNReal.coe_le_coe
  }
...
lemma bind_limit_sumR {α β Ω : Type} {l : Filter Ω} [l.NeBot]
  ...
  have h_tsum :
    ...
    generalize_proofs at *; (
    convert le_of_tendsto_of_tendsto' tendsto_const_nhds h_limit h_finite using 1
    rw [ prob_bot ];
    ring!;))
  generalize_proofs at *; (
